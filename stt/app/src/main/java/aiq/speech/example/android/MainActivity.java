...
public class MainActivity extends AppCompatActivity implements StreamObserver<StreamingRecognizeResponse> {
    ...
    private void startGrpc() throws Exception {
        ...
        final SpeechGrpc.SpeechStub stub = SpeechGrpc.newStub(channel).withCallCredentials(new CallCredentials() {
            @Override
            public void applyRequestMetadata(RequestInfo requestInfo, Executor appExecutor, MetadataApplier applier) {
                try {
                    final Metadata metadata = new Metadata();
                    metadata.put(Metadata.Key.of("x-api-key",
                            Metadata.ASCII_STRING_MARSHALLER), API_KEY);
        ...
        final StreamingRecognitionConfig streamingRecognitionConfig = StreamingRecognitionConfig.newBuilder()
                .setConfig(
                ...
                .build();
        // 데이타를 전송한다.
        stream.onNext(StreamingRecognizeRequest.newBuilder()
        ...
        final Runnable recorder = () -> {
            ...
            if (audioRecord.getState() == STATE_INITIALIZED) { // 정상적으로 생성되었다면
                ...
                while (isRunning) {
                    ...
                    // gRpc 데이타 객체
                    final StreamingRecognizeRequest req = StreamingRecognizeRequest.newBuilder()
    ...
    public void onNext(StreamingRecognizeResponse res) {
        ...
        res.getResultsList().forEach((StreamingRecognitionResult result) -> {
